...
## Project status: M0–M7 ⭐ + app-shell + design system shipped · next: M8 (notifications engine + saved searches)

> *Snapshot as of 2026-07-25 — the **live** status is `docs/milestones.md` § Progress tracker + `docs/progress.md` (or run `/resume`). Update this line at a milestone close so it doesn't rot.*

M0 scaffolded the app (`app/` React + Vite + TS SPA, `backend/` FastAPI + SQLModel, the root `package.json` + pytest/Vitest harness). **M1** shipped auth — register/login/JWT, `get_current_user` + `require_admin`, and the error contract (`errors.py` + handlers + request-id). **M2** shipped the seller listing builder + document uploads — the public/private table split (`Listing` vs `ListingPrivate`/`ListingDocument`), owner-scoped writes, the listing state machine, and the storage port. The throwaway M0 `SandboxItem`/`/api/sandbox` pair is **deleted**. **The app-shell foundation milestone** (`specs/pre-003-app-shell/`) wired the M1/M2 frontend components into a running, routed app — `/login`, `/register`, `/my-listings`, `/sell`, a public landing page, a nav bar, and the global-401 redirect; `App.tsx` is no longer the M0 health page. **The design-system pass shipped** (#28, owner-directed, no spec folder): `app/src/theme.ts` is the single source of token truth, `docs/design_system_spec.md` the decisions behind it, brand artwork in `docs/brand/`, `StatusChip` for the listing status vocabulary — and all six screens are built to it. **M3** shipped admin curation — the `require_admin` curation queue, approve/reject with a required reason (approve is the only path to `live`), the append-only `listingevent` audit trail, and the admin UI at `/admin`. **M4** shipped the public marketplace — the project's first unauthenticated routes (`GET /api/listings` + `/api/listings/{id}`), the `ListingPublic` boundary (a standalone model, never a `ListingRead` subclass), filters + keyword search over public text only, `seed/seed.py`, the `/browse` UI, and the succession-voice landing copy. The owner's listing view **moved** to `GET /api/my/listings/{id}` so the public view could take the canonical path. **M5 ⭐** shipped the **trust core** — `require_private_access` (the one gate guarding the private payload, the data-room index and the document downloads), the platform-wide NDA signed once per user, per-listing access requests with a unique constraint on `(listing, buyer)`, seller-only approve/deny/**revoke**, and the append-only `accessrequestevent` audit that revocation cannot erase. Two deliberate partials, both recorded on their FRs: FR-14's *verification status* half waits for M10, and notification events wait for M8. **M6** shipped the project's **first WebSocket surface** — `WS /ws/conversations/{id}` authenticates during the handshake and re-checks membership live (not cached) so a seller's revocation applies immediately to a socket already open, not just the next REST call; a `Conversation`/`Message` schema created only on M5 approval; a rate cap that counts every inbound frame, valid or not; and the REST reads (`GET /conversations`, message history, mark-as-read). **M7 ⭐** shipped the project's **first money surface + first bilateral state machine** — structured offers/LOI (`POST /listings/{id}/offers`, approved-access + `live` required), a counter-offer negotiation as a chain of linked `Offer` rows where decision rights follow whoever did **not** propose the current terms (spec 007 D1), the **atomic accept** (offer + listing flip + sibling auto-decline in one compare-and-swap transaction, so two accepts can't both win), buyer withdraw, and the append-only `offerevent` audit; its independent appsec pass found and closed a *counter-on-a-non-live-listing* bypass (a new priced row could be spawned on a paused/closed listing — the corridor past an open thread), recorded as D8. **Next: M8** (notifications engine + saved searches). Not yet built: notifications (M8) and everything after; Playwright E2E (Phase D). `nextowner.db` is created on first run (gitignored).
