...
- **At each milestone's close, the three status surfaces refresh themselves:** `/dod` step 6 (and `/run-milestone`) commits, on the feature branch once the review is clean and before the PR opens, the updated `progress.md` status + ▶ next action, the tick of the milestone's box in `docs/milestones.md` § Progress tracker, **and this file's `## Project status:` line** — so `main`'s copy of all three is correct the instant the PR merges. (The status line was added to this trigger on 2026-07-18 after it drifted stale for two milestones; it's kept as a dated snapshot pointing at the tracker so it degrades gracefully, and `/resume` cross-checks it against the tracker to catch any future slip.) **⚠ Known flaw — the trigger does not actually hold (2026-07-19).** It has now failed twice: PRs #26/#27 merged as small follow-ups without running `/dod` at all, and #28 ran it but wrote *"PR open — awaiting approval"*, which was true at commit time and false the moment it merged. Both failures share a cause: **the refresh is mounted on the milestone-close ceremony, and it asks the branch to record a claim that only becomes true later.** The fix is to bind it to the **merge** — the one step no change can skip, and the point at which "merged" is actually true (a merge hook, or a CI job on push to `main`). Until that exists, treat these three surfaces as *stale-by-default right after a merge* and check them against `git log main`.
