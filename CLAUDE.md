...
- **At each milestone's close, `docs/progress.md` and `docs/milestones.md` refresh themselves:** `/dod` step 6 (and `/run-milestone`) commits the updated `progress.md` status + ▶ next action, plus ticking the milestone's box in `docs/milestones.md` § Progress tracker, on the feature branch once the review is clean, before the PR opens — so `main`'s copy of both is correct the instant the PR merges, with no separate action needed.
