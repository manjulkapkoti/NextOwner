...
- **No PR until the branch review is clean** — the orchestrator reviews inline every milestone (architecture + the §8 matrix), plus **one** independent `appsec-engineer` pass on the security-critical milestones (M1/M2/M5/M7/M8/M10) — all **on the branch** (step 7), before the PR is opened. A PR here means "vetted, ready for a human." (Inline-first keeps context/usage low; the `/dod` forbidden-path tests are the always-on security floor.)
