...
4. **Create `specs/NNN-<name>/plan.md`** with:
   ...
   - **Build order** — the ordered implementation slices. Every other section above says *what will exist when this is done*; this one says *in what order it gets built*, so the milestone is worked one slice at a time instead of as one big-bang. Rules:
     - **A slice = one trust boundary** (constitution Article 2 #1: "one function per trust boundary"). Not one test (too fine), not one layer (too coupled). Let the slices fall out of the architecture: settings/config → model + first write path → the token/identity gate → each `permissions.py` gate → the guarded endpoints → frontend → cleanup.
     - **Each slice names the red tests it turns green** and ends in one Conventional Commit. Order by dependency, and say *why* each slice sits where it does — that one-line rationale is the whole value of the section.
     - **NO checkboxes, and no status of any kind.** `plan.md` fixes the **order** (a design decision — durable, reviewable, in git). The **red test list is the status** (`cd backend && pytest -q --lf`), and the red count is the progress bar. This is deliberate: `/resume` reconstructs work from git + the failing tests precisely *because* status files drift, and a ticked box in `plan.md` would be a second source of truth that lies after the first crash. Never add one.
     - Mid-milestone the full suite is **red by design** (step 3 wrote every test failing first) — that's the queue draining, not a broken build. `/dod` is the gate at the end; commit freely on the branch.
     - Call out any slice that **removes** tests rather than turning them green (e.g. deleting a throwaway endpoint deletes its tests). That must be an explicit spec criterion — `/dod` forbids editing tests to make them pass — and it almost always sequences **last**, so the suite doesn't go red for reasons unrelated to this milestone.
