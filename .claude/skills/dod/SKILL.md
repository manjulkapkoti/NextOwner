...
6. **When green + checklist complete, hand off to the branch review — do NOT open the PR yet.** The review happens **on the branch, before any PR exists** — a PR means "vetted, ready for a human." It's **inline-first**: the orchestrator reviews the diff itself (architecture + the `docs/security.md` §8 matrix + confirming the negative tests), reusing warm context. On the **security-critical milestones** (M1/M2/M5/M7/M10) *additionally* spawn a single diff-scoped `appsec-engineer` agent, **passing `model` explicitly — `model: "sonnet"`, or `model: "opus"` for M5.** (The agent's frontmatter defaults to opus; omit the parameter and the review pass silently runs on Opus.) See `docs/git_strategy.md` § Branch review. So on green: commit any final work with a Conventional Commit message, run the review, fix any blocking finding, and re-run `/dod`.
