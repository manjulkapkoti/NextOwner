...
6. **When green + checklist complete, hand off to the branch review — do NOT open the PR yet.** The review happens **on the branch, before any PR exists** — a PR means "vetted, ready for a human." It's **inline-first**: the orchestrator reviews the diff itself (architecture + the `docs/security.md` §8 matrix + confirming the negative tests), reusing warm context. On the **security-critical milestones** (M1/M2/M5/M7/M8/M10) *additionally* spawn a single diff-scoped `appsec-engineer` agent, **passing `model` explicitly — `model: "sonnet"`, or `model: "opus"` for M5.** (The agent's frontmatter defaults to opus; omit the parameter and the review pass silently runs on Opus.) See `docs/git_strategy.md` § Branch review. So on green: commit any final work with a Conventional Commit message, run the review, fix any blocking finding, and re-run `/dod`.

   **Refresh `docs/progress.md` and tick `docs/milestones.md` on the branch** — once the review is clean, update `progress.md`'s **Milestone status** (this milestone → shipped, pending merge), **▶ NEXT ACTION** (the next milestone's `/start-milestone` or `/run-milestone` command — or the next mid-milestone step, if this PR doesn't close the milestone), and **Carryover notes**. If this PR completes the milestone, also tick its box in `docs/milestones.md` § Progress tracker (`- [ ] **M<N>**` → `- [x] **M<N>**`). Commit both together (`docs: update progress.md + tick M<N> — <milestone> shipped`). This is the only reliable trigger point: branch protection blocks direct commits to `main`, and no local hook can observe a GitHub merge event, so baking both edits into the PR's own commits is what makes `main`'s copy correct the instant it merges. (`/checkpoint` still exists for manual saves mid-milestone.)
