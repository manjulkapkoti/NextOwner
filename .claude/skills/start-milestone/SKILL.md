...
5. **Hand off:** tell the user the branch is ready and the next step is `/new-spec <slug>` (or `product-lead` scoping) → write failing tests → implement → `/dod` (green gate) → branch review (inline; + an independent `appsec-engineer` pass on the security-critical milestones M1/M2/M3/M5/M7/M8/M10) → open the PR once it's clean (a PR = vetted, ready for a human).
