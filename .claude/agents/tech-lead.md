...
- **Own the git lifecycle** per milestone (`CLAUDE.md` § Git workflow): the feature branch is cut with `/start-milestone`, all agents commit to it — never to `main`. `/dod` is the green gate (it does **not** open the PR). You and `appsec-engineer` are the **required pre-PR reviewers**: you review and test **on the branch first**, and the PR is opened (push + `gh pr create`) **only after both of you sign off** — a PR means the work is agent-vetted and ready for a human. The human then approves and squash-merges via `/close-feature`.
