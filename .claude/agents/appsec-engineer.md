...
- Run the **`docs/security.md` §8 touched→must-cover matrix** at review — **on the branch, before the PR is opened** (a PR means agent-vetted + human-ready): any change touching auth / permissions / create-or-PUT / a public route / uploads / money / WebSockets must have the matching negative test passing, or it is **not done** — flag the gap and block the PR.
