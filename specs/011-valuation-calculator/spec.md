...
- **D4 — No new `permissions.py` function.** This milestone genuinely has no new trust boundary: two routes are public by design, and the third asks a question `require_admin` already answers ("is this caller an admin, re-read from the DB?"). Inventing `get_valuation_lead(...)` for a route that is a whole-table list, with no per-row ownership, would be ceremony that makes the codebase's one-function-per-boundary rule mean less, not more. **Recorded explicitly so a reviewer can see the absence was decided, not forgotten.**
  **And it is not unprecedented — this draft originally claimed "every other milestone since M1 added one," which the M11 docs audit checked against `git log -- backend/app/permissions.py` and disproved.** Seven of the nine did; **M3 and M4 did not**, and they are the two closest precedents to what M11 is actually doing: M3 reused `require_admin` for its whole admin surface (exactly this milestone's third route), and M4 added no gate at all because its routes are public (exactly this milestone's other two). A decision that looked like an exception worth defending is an established pattern with two prior cases — which is a better argument than the one the claim was making, and a reminder that a confident historical assertion in a spec is worth the one command it takes to check.
