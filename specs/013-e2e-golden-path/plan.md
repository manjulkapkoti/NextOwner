...
9. **A navigation race in `BrowseListings` — found by the path, and fixed (F7).** The search box debounces 250ms and then writes `q` to the URL; clicking a result inside that window navigates to the detail page and the debounce *then* fires from an unmounted component. `setParams` navigates, so the visitor is thrown back to the marketplace with a half-typed search applied. Fixed by cancelling the timer on unmount — one line, and the component's own fetch effect already guarded the identical hazard with a `cancelled` flag, so only the timer had been missed. Reachable by hand whenever results from a previous query are still on screen; the path just hits it more reliably than a human does. The path's `waitForURL(/[?&]q=/)` **stays** for a different and better reason: it is what makes G6 assert the buyer found the listing *by searching*, rather than clicking a card that was on screen already.
