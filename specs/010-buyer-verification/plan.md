...
|---|---|---|
| `POST /api/verification/documents` | `get_current_user` (D4 — no role gate) | Validates file (M2 rules, D5) + quota (D8); creates `BuyerVerificationDocument`; sets `verification_status` to `pending` from `unverified`/`rejected`; 409 if currently `verified` (D3); writes no event row for the upload itself (D6's `actor_id` note). `response_model=VerificationDocumentRead`, 201. **Three behaviours no criterion pins, decided during slice 1 and recorded here so they are choices rather than accidents:** *(i)* the D3 `already_verified` check runs **before** the file is read, so a refused request never writes bytes to disk — the visible consequence is that a `verified` buyer uploading a `.exe` gets 409, not 415; *(ii)* `pending → pending` is **allowed** (201, status unchanged, a second document recorded) — D1 names only `unverified`/`rejected` as from-states and is silent here, and a second document on a queued submission is more evidence rather than a new state, so refusing it would need a 409 code no criterion defines; *(iii)* re-queueing **clears** `verification_reason` and `verification_reviewed_at`, so a resubmitting buyer is not shown stale rejection text beside a freshly queued document — the history survives in `BuyerVerificationEvent`, which is exactly D6's justification, and it keeps this plan's own "`None` while `unverified`/`pending`" column note true. |
...
1. **Schema (`User` columns, `BuyerVerificationDocument`, `BuyerVerificationEvent`) + `POST /api/verification/documents` + `GET /api/verification`.** → **V1, V2, V10, V11, X1, X5, S1, S9** *(observed: 33 red → 25 red)*. *First because nothing else in this milestone is reachable without the ability to create a submission and read it back — the same "add must exist before anything else can be exercised" reasoning as spec 009's slice 1. S1 lands here too: it only needs the new `verification_status` column and the existing `PUT /profile` route, no new endpoint. S7's path-confinement reuse and V10/V11's whitelist/size-cap reuse cost nothing new (D5) — they're regression pins on the M2 code path applied to a new caller.*
   *Corrected against what this slice actually turned green, after it ran. Three of the criteria the first draft claimed here **cannot** go green until a later slice, all for the same reason — their **arrangement**, not their implementation, needs a route from slice 2 or 3: **V6** (resubmission) and **V7** (upload while `verified`) both have to reach a decided status first, so they move to slice 2; **S7** passes every confinement assertion here and fails only its closing round-trip through the slice-3 download route. Conversely **S9** lands here, not in slice 3 — it reads `GET /verification` and never touches the download route at all. The first draft reasoned about how cheap each criterion's *code* was and missed what each criterion's *setup* requires; that asymmetry is the whole lesson from M9's Build-order audit, and it is why this correction is re-derived from the observed red-count delta rather than re-guessed.*
2. **Admin queue + approve/reject (`require_admin`, D1's transitions, `BuyerVerificationEvent` writes).** → **V3, V4, V5, V6, V7, V14, X2, X3, X4, X6, S2, S3, S8, V15, S12** *(the last two were **written during this slice**, not before it — D13's `?status=` filter was added here, and its two criteria with it; they are the only criteria in the milestone that never had a red phase, which is why they were sabotage-verified instead)*. *Second because the state machine's only non-buyer-initiated transitions live here, and D1's "reject doubles as deny and revoke" can't be tested (V14) until both `pending→rejected` (V5) and `verified→rejected` (V14) are reachable from the same endpoint. S2/S3 fall out for free once the routes exist — the negative test is "the same route as V4/V5, called by the wrong identity."*
3. **`get_owned_or_admin_verification_document` + `GET /api/verification/documents/{id}`.** → **S4, S5, S6, S10, and S7's closing round-trip** *(S9 moved to slice 1 — see the correction there; **S6** belongs here and was missing from every slice list until the docs audit caught it — it sits at 5/6 from slice 1 onward and completes the moment this route mounts, since its sixth entry is this route's 401)*. *Depends on slice 1 for a document to fetch and slice 2 for an admin identity to test the "admin can view any" half (S5) meaningfully against a submission that has actually been queued. Kept separate from slice 1 because it is a distinct trust boundary (owner-or-admin, not just owner) — Article 2 #1's "one function per trust boundary" — not because of dependency order alone.*
...
**The six slice lists above account for all 33 criteria** — 8 + 15 + 5 + 3 + 2, with S7 counted in slice 3 though it is split across slices 1 and 3 (confinement, then the round-trip) and the frontend slice carrying none. That arithmetic is stated because the first three versions of this section did **not** add up: V15, S6 and S12 appeared in no slice at all, and the omission survived two rounds of correction that fixed other entries in the same paragraphs. It was caught by the independent docs audit, which checked each backend commit out in a worktree and re-ran the suite rather than reading the prose — the same method that caught M9's Build-order drift, now two milestones running. **A slice→test mapping is worth exactly as much as the last time someone ran it; if you edit this section, re-derive the total.**
