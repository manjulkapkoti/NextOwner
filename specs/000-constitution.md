...
>
> **This file holds the rules in force. The reasoning behind them — every amendment, with the evidence and the near-miss that prompted it — lives in [`000-constitution-amendments.md`](./000-constitution-amendments.md), which is *not* loaded by default. Read it when you need to know *why* a rule says what it says, when you are about to change a rule, or when a rule seems wrong.** Split 2026-07-25 for context efficiency; nothing was deleted, and every still-live rule that had no home in the Articles was promoted into them first. **Each promotion is marked *[promoted …]* with its actual source** — usually the amendment log, but three came from elsewhere, so read the tag rather than assuming.
>
...
5. **Audit what matters:** offers and access decisions get timestamped event rows. *[promoted 2026-07-25 from `docs/progress.md` § M5 build notes]* Before adding an event row, ask **what it preserves that the row itself loses** — an audit row earns its place only for values a later transition overwrites. Correspondingly, **adding a transition to a state machine can invalidate an audit design that was correct for the old one**; re-ask "what does this overwrite?" whenever a state machine grows.
...
3. **Definition of done:** a milestone is done when its tests pass **and** the full `npm test` suite is green. Work happens on a per-milestone feature branch off `main`; commit freely on the branch (including the failing-tests-first commit). **Before the PR is opened**, the work is reviewed and tested **on the branch** (a pre-PR gate): the orchestrator reviews inline every milestone (architecture + the security must-cover matrix), plus one independent `appsec-engineer` pass on the security-critical milestones (M1/M2/M3/M5/M7/M8/M10) — opening a PR signals the work is vetted and ready for human approval. `main` is updated **only by merging a PR that is green** (tests + the security must-cover matrix) — never by a direct commit, so `main` is always green.
   ...
   - *[promoted from the log, 2026-07-15]* The independent `appsec-engineer` pass is spawned **diff-scoped and in the background, with the model passed explicitly** — to keep context and usage low.
   - *[promoted 2026-07-25 from `scripts/check_docs_trigger.py` + `docs/git_strategy.md`]* The same shape applies to **every** independent review agent, `docs-auditor` included — that script decides from the diff when a docs audit is warranted. Give any review agent the diff **and** the binding documents: a contradiction lives between the diff and a file the diff never touched.
