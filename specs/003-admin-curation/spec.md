...
- **E6** — GIVEN any status a seller can be in, WHEN **any sequence of up to three seller-reachable actions** is applied (submit / pause / resume / edit, in every order), THEN no sequence containing a successful edit leaves the listing `live`.
  *The invariant, rather than another door.* E1–E5 each name one forbidden path, which is why the E5 bypass survived: it was a path nobody had thought to name. E6 asserts the property instead — *a seller cannot publish content an admin has not seen* — over the whole reachable graph. Verified by reintroducing the E5 bug: E6 fails with **six** distinct paths, including `paused → edit → resume`, which is shorter than the one the human reviewer found. Every seller route added by M4+ is checked against this for free.
