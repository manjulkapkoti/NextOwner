...
| **FR-22** | All notification templates are **centrally managed and event-driven** — one `notify()` service, one template table, no per-endpoint ad-hoc messaging. |
| **FR-16** | The **email fallback** half of buyer↔seller messaging (M6 shipped the realtime half and left this). |
| **FR-8** | "State changes propagate to search and alerts" — the propagation half. |
| **FR-1** | Self-serve password reset — the account-recovery half of email+password auth. |
| **NFR Scalability** | "fan-out alert jobs must handle publication spikes" — bounded per-user search count + the dispatch seam (§ Decisions D9). |
...
- **D9 — Dispatch sits behind a port.** `EmailSender` (Protocol) / `SmtpEmailSender` / `InMemoryEmailSender`, shaped exactly like `RateLimiterBackend` and `ChatBroker` — the codebase's third and fourth instances of "external/per-instance effect behind a swappable interface". The fan-out runs in a `BackgroundTask` now; the seam that lets a real queue replace it later is the same one. Tests never open a socket.
...
- **D12 — the fan-out runs *inside* the approve transaction, not in a `BackgroundTask`.** `design_implementation.md` Part 4's M8 sketch says "add a **BackgroundTask**"; this spec deliberately does not, and the sketch is annotated to match. Two reasons. **(a) Correctness:** a background task runs after the response, outside the transaction, so a publication that rolled back could still have alerted every matching buyer about a listing that is not live. Sharing the endpoint's transaction makes that impossible. **(b) It changes nothing observable** — `testing_guide.md` §5 already notes BackgroundTasks run synchronously under `TestClient`, so the tests are identical either way. The cost is that a very large subscriber base would slow an admin's approve; that is what the per-user cap (A9) and the dispatch seam (D9) exist to bound, and moving the fan-out to a real queue later is a change of *executor*, not of design.
...
- **S10** — GIVEN every code path that creates a notification, WHEN a breadth-first walk over reachable state sequences exercises them, THEN no notification ever exists whose recipient is not a party to the underlying object (the reachability invariant, in `test_nda_gate.py` D10's tradition).
