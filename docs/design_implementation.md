...
- In the approve endpoint (M3), add a **BackgroundTask**: after the listing goes live, match it against all saved searches and insert `notification` rows. *(M8 adds this to M3's endpoint — M3 itself ships no notification code; see below.)* `GET /notifications` powers an in-app inbox (poll it, or refetch on route change — fine for MVP).
