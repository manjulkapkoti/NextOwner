...
- Seller dashboard: **`GET /my/listings/{id}/access-requests`** with buyer profile → `POST /access-requests/{id}/approve|deny|revoke` (only the listing's seller may decide). *(Path amended 2026-07-20, owner-approved at M5 spec time — spec 005 D7. This read `GET /access-requests?listing_id=…`, written before **M4** established the `/my/` prefix for owner-scoped views. Putting the listing id in the path means the existing `get_owned_listing` dependency guards the route — a trust boundary already written, already tested, already returning 404-not-403 so a draft's existence stays hidden. The query-param form would have re-implemented that check by hand in the handler, which is what Article 2 #1 "one function per trust boundary" exists to prevent.)*
...
- In the approve endpoint (M3), after the listing goes live, match it against all saved searches and insert `notification` rows. *(M8 adds this to M3's endpoint — M3 itself ships no notification code; see below.)* `GET /notifications` powers an in-app inbox (poll it, or refetch on route change — fine for MVP). **Amended 2026-07-26 (spec 008 D12): this said "add a BackgroundTask", and what shipped runs the fan-out *inside the approve transaction* instead.** A background task runs after the response and outside the transaction, so an approval that rolled back could still have alerted every matching buyer about a listing that never went live. Nothing observable changes — a plain call and a `BackgroundTask` are both complete by the time the test asserts — so the §5 checklist's "assert right after the approve call" still holds, now for the simpler reason that there is no deferral at all. Moving the fan-out onto a real queue later is a change of executor, not of design. *(This sentence previously justified itself by citing `testing_guide.md` §5's note that "BackgroundTasks run synchronously under TestClient" — circular, since the whole point is that no BackgroundTask is used. That note has been corrected too.)*
...
- **Scope expanded (2026-07-16 gap review):** this milestone is also the general **notifications engine** (FR-22 + the FR-16 email fallback). Earlier milestones (M3/M5/M6/M7) leave the **event rows M8 projects notifications from** — listing approved/rejected, access requested/decided, new message, new offer — and M8 builds both the `notification` table and the delivery surface **(amended 2026-07-19, owner-approved: M3 writes `listingevent` and does NOT create or write a `notification` table. A table designed five milestones before its only consumer is speculative, and one written by M3 but read by nobody until M8 could not be verified by any test writable at M3 time. When scoping M8, start from `listingevent` and ask the same question of M5/M6/M7's events before inventing a schema — see `milestones.md` § Scope fold-ins → M8.)**  (the in-app inbox above + email) for **all** of them, alongside the saved-search fan-out. Without these events the two-sided loop stalls until someone happens to log in. Every delivery is caller-scoped.
- **Account lifecycle lands here too (moved from M1, 2026-07-17):** `POST /auth/forgot-password` + `POST /auth/reset-password`, and email verification at registration. **Why here and not M1:** these flows *are* email, and M8 is where the email channel is built — M1's fold-in had the dependency backwards ("the channel arrives in M1; M8 reuses it"). Moving them made M1 right-sized (it was M1's only heavy item) and made **M8 security-critical**: a reset token is account takeover if it leaks (`security.md` §7 M8 has the rules — single-use, short expiry, hashed at rest, no user-enumeration, never logged). **Accepted trade-off:** M1–M7 have no self-serve password reset and no verified addresses — fine while the build is 100% local with no real users, not fine after that.
