...
- **FR-14** Sellers can approve/deny access requests and see buyer profile/verification status before approving. **The verification half is (Deferred to M10)** — *owner-approved 2026-07-20 at M5 spec time (spec 005 D5).* M5 ships the **profile** half only: display name, budget, target industries, experience. There is no `verified` field anywhere in the codebase — **M10** owns buyer verification and its scope fold-in already covers surfacing the badge in this list. Shipping a placeholder at M5 would have been an unenforced flag, which `security.md` §7 calls decoration, and no test writable at M5 time could have verified it. **So FR-14 is partially satisfied until M10 lands** — recorded here rather than only inside a spec folder, because this file is where a reader checks whether a requirement is met.
