...
### Auth & profiles
- **FR-1** Users can register/sign in with **email + password**; sessions use **short-lived access tokens** *(refresh: deferred — see the tail note)*. **Google OAuth is (Post-MVP)** — *marked 2026-07-17: this clause read "email-password **and** Google OAuth" with no deferral marker, while `design_implementation.md` Part 4 (M1) had already ruled it "a post-MVP nicety — email/password teaches the full mechanics first." The binding doc and the build guide disagreed, and only the build guide said so. Two reasons it stays deferred: the learning goal (own the JWT mechanics first — Article 1 issues JWTs from our own endpoints with bcrypt), and a hosted OAuth provider needs a cloud project, which Article 1's "**100% local — no cloud account**" rule excludes for now. Re-scoping it means amending this FR deliberately.* (The **refresh-token** half is **deferred 2026-07-17 — owned, not limbo**: refresh tokens + the httpOnly-cookie session they need land with the **deploy-hardening pass** (`security.md` §9), triggered by *a real deployment / real users*. M1 ships **access-token-only** and **records its production token-storage approach in the spec** — the seam that keeps refresh a clean later-add. This is the deliberate FR amendment the M1 fold-in demanded, not drift.)
...
- **FR-13** A buyer signs the standardized platform NDA once (click-wrap, timestamped on their account); thereafter they request access per listing, each request timestamped per buyer-listing pair. *(Amended 2026-07-13: platform-wide NDA adopted from Baton research; Acquire itself uses per-listing auto-signed NDAs.)* **Re-requesting after a decision is (Post-MVP)** — *owner-approved 2026-07-20, at M5 spec time.* The one-row-per-pair rule above is enforced as a **unique constraint**, which makes a decided request **terminal**: the state machine runs strictly forward (`requested → approved|denied`, `approved → revoked`), and a denied or revoked buyer re-requesting gets **409**. **The accepted consequence:** a seller who revokes by mistake, or reconsiders a denial, has no in-product way back — the pair is spent. Deferred rather than improvised inside the security milestone because the answer is a product question, not a technical one (does a revoked buyer get a second chance; who initiates it; does the seller re-approve or the buyer re-ask), and each answer implies a different transition into a state the gate currently treats as final. **Whoever picks this up owns three things:** the new transition + its forbidden-path twin, whether the unique constraint survives (a re-request either reuses the row or needs the constraint relaxed — the second weakens FR-13's per-pair audit guarantee), and a **rate limit on the request endpoint** if buyers can re-ask, since nothing today bounds nagging a seller (M1's limiter covers auth only — `security.md` §9). Recorded in `specs/005-nda-gate/spec.md` § Out of scope (D3) with the reasoning at the decision point.
- **FR-14** Sellers can approve/deny access requests and see buyer profile/verification status before approving. **✅ Fully satisfied since M10 (2026-07-28).** The two halves landed five milestones apart, deliberately: M5 shipped the **profile** half (display name, budget, target industries, experience) and *deferred* verification — *owner-approved 2026-07-20 at M5 spec time (spec 005 D5)* — because a placeholder `verified` flag would have been an unenforced field, which `security.md` §7 calls decoration, and no test writable at M5 time could have verified it. **M10 closed it** (spec 010 D7): `BuyerProfile` now carries `verification_status` and a computed `verified`, read live off the buyer's record so a revoked badge cannot go stale, and because `BuyerProfile` is one model the badge reaches M7's offer queue as well as this list. Pinned by spec 010 V8 (a verified buyer reads as verified here) and S11 (a revoked one does not). *This paragraph is kept in full rather than trimmed to "satisfied": the deferral was the right call and the reasoning is why the field arrived when it did — and this file is where a reader checks whether a requirement is met, so it is also where a stale "partially satisfied" would have misled them longest.*
...
- **FR-17** Buyers can submit a structured offer/LOI (price, structure, contingencies, close date); sellers can accept, decline, or counter; both parties see offer history. **Decision rights follow the proposer, not a fixed role** — *owner-approved 2026-07-21 at M7 spec time (spec 007 D1).* "Sellers can accept, decline, or counter" describes the buyer-initiated path (where the money-moving accept lives); on a seller's **counter**, the buyer may accept/decline/counter-back. The rule the code enforces: whoever did **not** propose the current terms decides them. Recorded here because this file is where a reader checks what a requirement actually promises.
