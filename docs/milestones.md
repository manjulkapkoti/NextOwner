...
- **M5 — NDA + access gate**
  ...
  - **The access-request list shows the buyer profile** (FR-14 — depends on the M1 profile fold-in). **The *verification status* half was deferred to M10** *(owner-approved 2026-07-20, spec 005 D5)*: no `verified` field existed yet, M10 owned it, and M10's own fold-in below covered surfacing the badge here. M5 therefore satisfied FR-14 partially and said so. **Closed 2026-07-28 — M10 shipped the badge on this exact route (spec 010 D7, V8/S11), so FR-14 is now whole.**
...
- **M10 — buyer verification:** the badge surfaces on the M1 profile (and in M5's request list); add a **per-listing upload count / total-size quota** (extends the M2 upload rules; security.md §6 addendum).
  - **Both honoured, and one deliberately wider than asked** *(shipped 2026-07-28, spec 010)*. The badge landed on `UserRead` and `BuyerProfile` — and because `BuyerProfile` is **one model**, it reaches M7's offer queue as well as M5's request list. That was a choice, not a spillover: same seller, same buyer, a *more* consequential decision. The alternative (defaulting the fields so `offers.py` compiled untouched) would have reported every buyer as unverified on the money surface, and a badge that lies is worse than no badge. The quota is likewise applied to **both** upload routes: read literally, "per-listing" is M2's `POST /listings/{id}/documents`, which had no count cap at all, so narrowing the fold-in to only the new route would have dropped the gap it exists to name. One shared check, keyed on the owning entity.
  - **Two things this fold-in did not anticipate, both recorded in spec 010:** a `?status=` filter on the admin queue (**D13** — story 4's revoke acts on an already-`verified` buyer, so a pending-only queue left it reachable only by an admin who already knew the user id), and the quota's collision with the resubmission promise (**D11** — resolved by choosing generous defaults rather than adding a deletion mechanism, with the lockout edge at the cap accepted and written down).
- **M11 — valuation calculator:** **email lead capture** (FR-23's business half — the calculator exists to capture seller leads) — or explicitly de-scope it in the spec.
  - **Taken, not de-scoped** *(shipped 2026-07-28, spec 011)*. It landed as its **own route** rather than an optional `email` field on the calculate route (D2), and that split is the milestone's security design: the route every visitor hits stores nothing, so anonymous traffic can never become a PII-collection or storage-exhaustion surface, while the one writer carries a much harder cap. The fold-in's implied third piece — somewhere for the leads to *go* — was added as `GET /api/admin/valuation-leads` (D3), on the M8 lesson that a table written before its only consumer cannot be verified by any test writable at the time.
  - **What the fold-in did not anticipate:** that "capture email as a lead" says nothing about whether to *mail* that address. Spec §8 de-scopes sending deliberately — M8 makes it a one-liner, but mailing an address captured on a public form is a consent/CAN-SPAM/GDPR decision for `legal-compliance`, not a build milestone. Worth noting that `specs/pre-011-rate-limiting/spec.md` had forecast M11 as "an unauthenticated form that causes outbound email"; the limiter it argued for still applies, but the severity is lower than predicted.
